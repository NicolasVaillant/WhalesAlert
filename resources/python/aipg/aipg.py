...
with open("./resources/config_python/aipg/tweet.json", "r") as f:
...
def get_transaction_info() -> list[Tuple[float, float, str]]:
    ...
    supply_url: str = "https://explorer.aipowergrid.io/ext/getmoneysupply"
    try:
        supply_response = requests.get(supply_url)
        supply_data = supply_response.text
        circulating_supply = float(supply_data)

        url: str = "https://explorer.aipowergrid.io/ext/getlasttxs/100/0/100"
        url_block : str = "https://explorer.aipowergrid.io/api/getblock?hash="
        url_tx: str = "https://explorer.aipowergrid.io/block/"
            ...
            if block['amount'] > 50000 : 
                ...
                for tx in block_data_json['tx']:
                    if tx not in seen_transactions:  # Check if we've seen this transaction
                        tx_get = requests.get(f'https://explorer.aipowergrid.io/api/getrawtransaction?txid={tx}&decrypt=1')
...
def post_tweet(payload: dict) -> None:
    ...
    with open("./resources/config_python/aipg/config.json") as f:
...
def send_telegram_message(message):
    with open("./resources/config_python/aipg/telegram.json") as f:
        config: dict = json.load(f)
...
def job_aipg() -> None:
    ...
    for transaction in transactions:
        ...
        message = "🐋 Whale Alert! 🚨\n"
        message += f"A transaction of {total_out_str} $AIPG "
    ...
    with open("./resources/config_python/aipg/tweet.json", "w") as f:
